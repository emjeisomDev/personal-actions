...
function mapWeeklyAssessment(
    row: WeeklyAssessmentRow
): WeeklyAssessment {
    return {
        id: row.id,
        studyAreaWeekId:
            row.study_area_week_id,
        weekGoal:
            row.week_goal,
        minutesStudied:
            row.minutes_studied,
...
export class WeeklyAssessmentRepository {
    ...
    public async findById(
        ...
        const result =
            await this.database.query<WeeklyAssessmentRow>(
                `
                SELECT
                    id,
                    study_area_week_id,
    ...
    public async create(
        ...
        if (!row) {
            throw new Error(
                'Weekly assessment was not created.'
            );
        }

        return mapWeeklyAssessment(row);
    }
